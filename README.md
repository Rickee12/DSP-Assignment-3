...
說明:
- ##### polyphase_decompose使用原因: 因為upsample時會再輸入之間差L-1個0，那因為convolution的運算是輸入和頻率響應相乘後再不斷累加，所以這些為0的值去做convolution基本上就是浪費時間和計算量，而且之後做downsample時也會把大部分的值丟棄，那為了解決這個問題才會使用polyphase_decompose，以跳過某些頻率響應的值以減少花費的時間及計算量
